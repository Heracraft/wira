...
import { CognitoToken, UserRecord } from "@/types/auth";
...
export default function page() {
	...
	const onSubmit: SubmitHandler<Record<string, any>> = async (data) => {
		try {
			let res = await signin(watch("username"), watch("password"));
			if (res.status === 200) {
				// console.log(res.idToken);

				// localStorage.setItem("token", res.idToken);

				// const congintoToken: CognitoToken = jwtDecode(res.idToken);

				const user = await getCurrentUser();

				console.log({ user });

				// if (congintoToken.email_verified) {
				setUser(user);
				// 	route.push("/dashboard");
				// }

				if (user.firstName === "John" && user.lastName === "Doe") {
					route.push("/auth/onboarding");
				} else {
					route.push("/dashboard");
				}
				// userStore(state=>state.setUser(res.data));
			} else {
							...
							<div className="grid gap-6">
								...
								<div className="grid gap-6">
									...
									<div className="grid gap-2">
										...
										<Input {...register("username", { required: true })} id="username" type="text" required autoComplete="username webauthn" />
										{errors.username && (
									...
									{errorOccured && <p className="text-xs text-destructive">An error occurred. Please try again</p>}
								</div>
