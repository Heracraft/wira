...
export default async function ExperienceTable({ profileId, limit }: { profileId: number; limit: number }) {
	const entries = await db
		...
		.from(workExperienceEntries)
		.where(eq(workExperienceEntries.profileId, profileId))
		.limit(limit);

	const headerItems = ["Position", "Company", "Start Date", "End Date"];

	if (entries.length === 0) {
		return <div className="text-center text-neutral-500 dark:text-neutral-400">No work experience found.</div>;
	}
